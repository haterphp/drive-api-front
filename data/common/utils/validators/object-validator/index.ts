...
type IValueWithEach<
  TEach extends boolean,
  TObject extends Record<any, any>
> = TEach extends false ? TObject : TObject[];
...
> extends AbstractValidator {
  ...
  public override validate(
    port: IValueWithEach<TEach, Partial<TObject>>
  ): IValidationError {
    let result:
      | PartialRecord<TKeys, ValidationError>
      | PartialRecord<TKeys, ValidationError>[];
  ...
  private _validateArrayOfObjects(
    port: PartialRecord<TKeys, unknown>[]
  ): PartialRecord<TKeys, ValidationError>[] {
    return port
      .map(this._validateObject.bind(this))
      .filter(this._objectHasError.bind(this));
  }
