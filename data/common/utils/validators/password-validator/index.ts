...
class PasswordValidator extends AbstractValidator {
  ...
  public override validate(value: string): IValidationError {
    const validations: { validationResult: boolean; code: number }[] = [
      {
        validationResult: this._isExistUpperLetter(value),
        code: ValidationCode.INVALID_LETTER_UPPER,
      },
      {
        validationResult: this._isExistLowerLetter(value),
        code: ValidationCode.INVALID_LETTER_LOWER,
      },
      {
        validationResult: this._validateLength(value.length),
        code: ValidationCode.INVALID_LENGTH,
      },
      {
        validationResult: this._validateSpecialSymbol(value),
        code: ValidationCode.NEED_SYMBOL,
      },
      {
        validationResult: this._isExistNumbers(value),
        code: ValidationCode.INVALID_NUMBERS,
      },
      {
        validationResult: this._validateInvalidSymbol(value),
        code: ValidationCode.INVALID_SYMBOL,
      },
      {
        validationResult: this._isExistCyrillicLetter(value),
        code: ValidationCode.INVALID_CYRILLIC,
      },
    ];

    const filteredErrorCodes = validations.filter(
