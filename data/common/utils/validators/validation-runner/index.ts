...
type IValidationErrors<TKeys extends RecordKey> = Partial<
  Record<TKeys, IValidationError["errors"]>
>;
...
> {
  private _validatorFactory: IValidatorsFactory<
    TPort,
    TValidators,
    TValidationErrors
  >;
  ...
  constructor(
    validators: IValidatorsFactory<TPort, TValidators, TValidationErrors>
  ) {
  ...
  public validate(port: TPort): void {
    ...
    for (const [key, _] of Object.entries(
      this._validatorFactory.properties
    ) as [keyof TPort, unknown][]) {
